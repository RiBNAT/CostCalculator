...
export class AuthService {
  ...
  /** Public client config — tells the SPA whether to render the Google button. */
  googleConfig(): Observable<{ googleEnabled: boolean; googleClientId: string }> {
    return this.http.get<{ googleEnabled: boolean; googleClientId: string }>(`${environment.apiBase}/auth/config`);
  }

  loginWithGoogle(idToken: string): Observable<{ user: User; tokens: TokenPair }> {
    return this.http
      .post<{ user: User; tokens: TokenPair }>(`${environment.apiBase}/auth/google`, { idToken })
      .pipe(tap((r) => this.store(r.user, r.tokens)));
  }

  private inflightRefresh: Observable<{ tokens: TokenPair }> | null = null;
  ...
  refresh(): Observable<{ tokens: TokenPair }> {
    if (!this.inflightRefresh) {
      this.inflightRefresh = this.http
        ...
        .pipe(
          tap((r) => this.storeTokens(r.tokens)),
