...
describe('AuthService', () => {
  ...
  it('stores tokens and user on Google sign-in', () => {
    service.loginWithGoogle('id-token-xyz').subscribe();
    const req = http.expectOne(`${environment.apiBase}/auth/google`);
    expect(req.request.method).toBe('POST');
    expect(req.request.body).toEqual({ idToken: 'id-token-xyz' });
    req.flush({
      user: { id: 'u2', name: 'G User', email: 'g@example.com' },
      tokens: { accessToken: 'gacc', refreshToken: 'gref' },
    });

    expect(service.accessToken).toBe('gacc');
    expect(service.isLoggedIn).toBeTrue();
    expect(service.user()?.email).toBe('g@example.com');
  });
