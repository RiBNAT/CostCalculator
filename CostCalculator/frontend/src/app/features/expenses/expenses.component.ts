...
import { Account, Category, Expense, RecurringExpense } from '../../core/models';
import { MoneyPipe } from '../../core/money.pipe';
import { categoryColor } from '../../core/cat-color';
import { saveBlob } from '../../core/save-blob';
import { defaultEntryDate } from '../../core/dates';
...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, RouterLink, MatButtonModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatIconModule,
    ...
    <div class="page">
      <div class="head-row">
        ...
        <div class="head-actions">
          <button mat-stroked-button [matMenuTriggerFor]="recMenu" [disabled]="closed() || !state.selected()">
            <mat-icon>repeat</mat-icon> Recurring
          </button>
          <mat-menu #recMenu="matMenu">
            @for (r of recurring(); track r.id) {
              <button mat-menu-item (click)="addFromRecurring(r)">
                <span>{{ r.label }}</span>
                <span class="rec-amt">{{ r.amount | money }}</span>
              </button>
            } @empty {
              <span class="rec-empty">No templates yet</span>
            }
            <a mat-menu-item routerLink="/settings" [queryParams]="{ tab: 'recurring' }">
              <mat-icon>tune</mat-icon> Manage templates…
            </a>
          </mat-menu>
          ...
          <tbody>
            @for (e of paged(); track e.id) {
              <tr (dblclick)="openDialog(e)">
                ...
                <td class="remarks" [matTooltip]="e.remarks ?? ''" matTooltipShowDelay="300">{{ e.remarks }}</td>
                <td class="actions">
  ...
  styles: [
    `
      ...
      .rec-amt { margin-left: 14px; color: var(--ink-soft); font-variant-numeric: tabular-nums; }
      .rec-empty { display: block; padding: 8px 16px; color: var(--ink-soft); font-size: 13px; }
    `,
  ],
})
export class ExpensesComponent {
  ...
  /** One-tap add an expense from a saved template, dated within the period. */
  addFromRecurring(r: RecurringExpense): void {
    const p = this.state.selected();
    if (!p || this.closed()) return;
    this.api
      .createExpense(p.id, {
        date: defaultEntryDate(p),
        categoryId: r.categoryId,
        subcategory: r.subcategory,
        accountId: r.accountId,
        amountExpr: String(r.amount / 100),
      })
      .subscribe({
        next: () => {
          this.snack.open(`Added ${r.label}`, undefined, { duration: 2000 });
          this.reload(p.id);
          this.state.refreshSelected();
        },
        error: (e) => this.snack.open(e?.error?.error?.message ?? 'Add failed', 'OK', { duration: 4000 }),
      });
  }
  ...
  remove(e: Expense): void {
    ...
    this.confirm
      .confirm({
        ...
        message: `${this.catName(e.categoryId)} · ${e.subcategory} — ৳${(e.amount / 100).toLocaleString('en-IN')} on ${e.date.slice(0, 10)}. This cannot be undone.`,
      })
      .subscribe((ok) => {
        if (!ok) return;
        this.api.deleteExpense(p.id, e.id).subscribe(() => {
