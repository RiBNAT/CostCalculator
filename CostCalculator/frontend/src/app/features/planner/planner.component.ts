        ...
        <div class="card">
          ...
          @for (w of windows(); track w.id) {
            <div class="row">
              ...
              <button mat-icon-button (click)="removeWindow(w)" aria-label="delete payment window"><mat-icon>close</mat-icon></button>
            </div>
          } @empty {
            <div class="empty-hint">No payment windows yet — add one above to track when a bill is due.</div>
          }
        </div>
        ...
        <div class="card">
          ...
          @for (r of reminders(); track r.id) {
            <div class="row" [class.done-row]="r.done">
              ...
              <button mat-icon-button (click)="removeReminder(r)" aria-label="delete reminder"><mat-icon>close</mat-icon></button>
            </div>
          } @empty {
            <div class="empty-hint">No reminders — add one above to get nudged before a due date.</div>
          }
        </div>
      </div>
    </div>
...
export class PlannerComponent {
  ...
  removeWindow(w: PaymentWindowWithStatus): void {
    const p = this.state.selected();
    this.confirmSvc
      .confirm({ title: 'Delete payment window?', message: `"${w.name}" will be removed from this period.` })
      .subscribe((ok) => {
        if (!ok) return;
        this.api.deleteWindow(w.id).subscribe(() => p && this.reloadWindows(p.id));
      });
  }
  ...
  removeReminder(r: Reminder): void {
    this.confirmSvc
      .confirm({ title: 'Delete reminder?', message: `"${r.task}" will be removed.` })
      .subscribe((ok) => {
        if (!ok) return;
        this.api.deleteReminder(r.id).subscribe(() => this.reloadReminders());
      });
  }
}
