          ...
          <mat-tab [label]="tab.label">
            <div class="lend-list">
              @for (l of byType(tab.key); track l.id) {
                ...
                <div class="empty-hint">No lends in this list — add one with the form above.</div>
              }
            </div>
          </mat-tab>
        }
...
export class LendsComponent {
  ...
  remove(l: Lend): void {
    this.confirmSvc
      .confirm({
        title: 'Delete lend?',
        message: `${l.person} — ৳${(l.amount / 100).toLocaleString('en-IN')} with ${l.settlements.length} settlement(s). This cannot be undone.`,
      })
      .subscribe((ok) => {
        if (!ok) return;
        this.api.deleteLend(l.id).subscribe(() => {
          this.snack.open('Lend deleted', undefined, { duration: 2000 });
          this.reload();
        });
      });
  }
