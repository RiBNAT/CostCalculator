...
// Marker identifies a workbook as a Cost Calculator monthly template (Config!A1).
// The legacy RIBNAT value is kept so templates downloaded before the rename
// still import.
...
func Generate(categories []domain.Category, accounts []domain.Account) (*excelize.File, error) {
	...
	f.SetCellValue(SheetConfig, "A2", "Cost Calculator monthly template — fill the green cells, then upload on the Import page. Amounts are in taka.")
