...
func TestImportRealWorkbook(t *testing.T) {
	...
	defer cancel()
	dbName := fmt.Sprintf("costcalc_test_%d", time.Now().UnixNano())
	...
	// The workbook's Dec 25 sheet has a stale daily panel that duplicates
	// Nov 25's range; the importer must fall back to its expense dates and warn.
	var nov25, dec25 *domain.Period
	for i := range all {
		switch all[i].Name {
		case "Nov 25":
			nov25 = &all[i]
	...
	if nov25 == nil || dec25 == nil {
		t.Fatal("Nov 25 / Dec 25 periods missing")
	}
	if dec25.StartDate.Equal(nov25.StartDate) {
		t.Errorf("Dec 25 still duplicates Nov 25's start date %s", nov25.StartDate)
	}
	decWarned := false
	for _, s := range report.Sheets {
		if s.Sheet == "Dec 25" && len(s.Warnings) > 0 {
			decWarned = true
		}
	}
	if !decWarned {
		t.Error("Dec 25 duplicate panel range imported without a warning")
	}
