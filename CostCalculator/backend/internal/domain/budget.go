...
// RolloverItems turns a previous period's *unspent* budget into extra budget
// items for the next period: one item per line whose budget exceeded its actual,
// for the positive leftover. Overspent lines carry nothing forward.
func RolloverItems(prevItems []BudgetItem, prevExpenses []Expense, accounts []Account) []BudgetItem {
	rep := BudgetReport(prevItems, prevExpenses, accounts)
	var out []BudgetItem
	for _, l := range rep.Lines {
		if l.Remaining > 0 {
			out = append(out, BudgetItem{CategoryID: l.CategoryID, Subcategory: l.Subcategory, Amount: l.Remaining})
		}
	}
	return out
}
